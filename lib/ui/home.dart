...
import 'package:penjualankucing/ui/inputpenjualan.dart';
import 'dart:convert';
import 'dart:async';
...
class _HomeState extends State<Home> {
  ...
  Future<List> getData() async {
    final response =
        await http.get("http://192.168.43.1/apiflutter/api/Penjualankucing");
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: new AppBar(
        title: new Text("Penjualan Kucing"),
      ),
      body: createListView(),
      //tombol add
  ...
  Future<http.Response> deletePenjualan(id) async {
    final http.Response response = await http
        .delete('http://192.168.43.1/apiflutter/api/Penjualankucing/delete/$id');
    return response;
  }
